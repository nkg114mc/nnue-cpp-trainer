...
void run_validation(std::string val_fn, int64_t epoch_size, torch::Device &main_device, NNUEModel &model, MyFileLogger &trlogger) {
    ...
    SparseBatchStreamWrapper stream;
    stream.create("HalfKP", 32, val_fn.c_str(), batch_size, true, true, random_fen_skipping, false);
    ...
    for (int batch_id = 0; batch_id < batch_cnt; batch_id++)
    {
        ...
        SparseBatchTensors batch_tensors(stream.next(), &main_device);
        ...
        batch_tensors.free_sparse_batch();
    }
}
...
*/
    ...
    MyFileLogger txtlogger("./training_output_epoch600.log");
    MyTensorBoardLogger trlogger("/home/mc/sidework/mycpptensorbd/testlogs", "");
    ...
    SparseBatchStreamWrapper stream;
    stream.create("HalfKP", 32, train_fn.c_str(), batch_size, true, true, random_fen_skipping, false);
...
*/
            ...
            */
            ...
            if (iter_id % 10 == 0) {
                txtlogger.log_txt("Loss: epoch=" + std::to_string(epoch) + " batch=" + std::to_string(iter_id) + " loss=" + std::to_string(loss.item<float>()) + "\n", true);
            }
            trlogger.log_scalar("train_loss", iter_id, loss.item<float>());
...
void training_speed_benckmark()
{
    ...
    SparseBatchStreamWrapper stream;
    stream.create("HalfKP", 32, train_fn.c_str(), batch_size, true, false, 0, false);
    ...
    for (int batch_id = 0; batch_id < batch_cnt; batch_id++)
    {
        ...
        SparseBatchTensors batch_tensors(stream.next(), &cuda_device);
